...
class InitialSpectrum:
    ...
    def __plot(self):
        ...
        arr_for_plot = self.__crop_arr(self.__arr_norm, 0.5)
        spectrum_for_plot = self.__crop_arr(self.__arr_norm, 0.5)

        ax1.contourf(arr_for_plot, cmap=cm.jet, levels=100)
        ax2.contourf(spectrum_for_plot, cmap=cm.gray, levels=100)

        plt.savefig('scripts/angular_spectrum/initial_spectrum/fft_vortex.png', bbox_inches='tight')
        plt.show()
        plt.close()
...
initial_vortex = InitialSpectrum(n_perp=1024,
                                 perp_max=1000 * 10**-6,
                                 m=1,
                                 r_0=15 * 10**-6)
